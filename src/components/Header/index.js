...
const toggle = () =>{
  ...
  return (
    <header className="w-full p-4  px-5 sm:px-10 flex items-center justify-between">
        ...
        fixed top-6 right-1/2 translate-x-1/2 bg-light/80 backdrop-blur-sm z-50">
            <Link href="/" className="mr-2">School/Department</Link>
            <Link href="/about" className="mx-2">About Me</Link>
            <Link href="/contact" className="mx-2">Industry</Link>
            <Link href="/" className="mr-2">OJT Blog</Link>
            <Link href="/about" className="mx-2">Ed Tour</Link>
            <Link href="/contact" className="mx-2">Research Fest</Link>
            <Link href="/contact" className="mx-2">Training/Seminars</Link>
            <button onClick={() => setMode(mode === "light" ? "dark" : "light")  }
            className={cx("w-6 h-6 ease ml-2 flex items-center justify-center rounded-full p-1", mode === "light" ? "bg-dark text-light" :
            "bg-light text-dark" )}
            aria-label="theme-switcher"
